...
exclude_files = {zip_filename, 'tsconfig.tsbuildinfo'}
...
print(f"Creating {zip_filename} including .git history and lockfiles...")

with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
    for root, dirs, files in os.walk('.'):
        dirs[:] = [d for d in dirs if d.lower() not in exclude_dirs]
        for file in files:
            # Block any .env or .env.* files (allow only example.env)
            if file.startswith('.env') or file in exclude_files or file.endswith('.pyc'):
                continue
...
print(f"==================================================")
print(f"ZIP ARCHIVE SUCCESSFULLY CREATED")
print(f"==================================================")
print(f"Archive Path           : {os.path.abspath(zip_filename)}")
print(f"Total Packaged Files   : {total_files} (including {git_files} git objects, {lock_files} lockfiles)")
