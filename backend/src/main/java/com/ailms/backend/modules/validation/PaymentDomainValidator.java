...
/**
 * Enterprise production validator for Payment domain invariants.
 */
@Component
public class PaymentDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Payment payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Payment: " + String.join(", ", res.errorMessages()));
        }
    }
}
