...
/**
 * Enterprise production validator for Enrollment domain invariants.
 */
@Component
public class EnrollmentDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Enrollment payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Enrollment: " + String.join(", ", res.errorMessages()));
        }
    }
}
