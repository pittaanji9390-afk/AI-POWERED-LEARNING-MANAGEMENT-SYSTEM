...
/**
 * Enterprise production validator for LearningPath domain invariants.
 */
@Component
public class LearningPathDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("LearningPath payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for LearningPath: " + String.join(", ", res.errorMessages()));
        }
    }
}
