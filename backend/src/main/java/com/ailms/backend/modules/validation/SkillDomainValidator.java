...
/**
 * Enterprise production validator for Skill domain invariants.
 */
@Component
public class SkillDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Skill payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Skill: " + String.join(", ", res.errorMessages()));
        }
    }
}
