...
/**
 * Enterprise production validator for Discussion domain invariants.
 */
@Component
public class DiscussionDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Discussion payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Discussion: " + String.join(", ", res.errorMessages()));
        }
    }
}
