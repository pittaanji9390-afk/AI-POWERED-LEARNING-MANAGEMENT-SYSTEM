...
/**
 * Enterprise production validator for Course domain invariants.
 */
@Component
public class CourseDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Course payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Course: " + String.join(", ", res.errorMessages()));
        }
    }
}
