...
/**
 * Enterprise production validator for Organization domain invariants.
 */
@Component
public class OrganizationDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Organization payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Organization: " + String.join(", ", res.errorMessages()));
        }
    }
}
