...
/**
 * Enterprise production validator for Certificate domain invariants.
 */
@Component
public class CertificateDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Certificate payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Certificate: " + String.join(", ", res.errorMessages()));
        }
    }
}
