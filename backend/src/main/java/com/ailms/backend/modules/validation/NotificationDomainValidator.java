...
/**
 * Enterprise production validator for Notification domain invariants.
 */
@Component
public class NotificationDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Notification payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Notification: " + String.join(", ", res.errorMessages()));
        }
    }
}
