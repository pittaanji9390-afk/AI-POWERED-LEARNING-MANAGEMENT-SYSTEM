...
/**
 * Enterprise production validator for LearningProgress domain invariants.
 */
@Component
public class LearningProgressDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("LearningProgress payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for LearningProgress: " + String.join(", ", res.errorMessages()));
        }
    }
}
