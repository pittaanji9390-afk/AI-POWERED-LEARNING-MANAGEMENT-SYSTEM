...
/**
 * Enterprise production validator for User domain invariants.
 */
@Component
public class UserDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("User payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for User: " + String.join(", ", res.errorMessages()));
        }
    }
}
