...
/**
 * Enterprise production validator for Quiz domain invariants.
 */
@Component
public class QuizDomainValidator {
    ...
    public ValidationResult validate(Map<String, Object> attributes) {
        ...
        if (attributes == null || attributes.isEmpty()) {
            errors.add("Quiz payload cannot be null or empty.");
    ...
    public void enforceValid(Map<String, Object> attributes) {
        ...
        if (!res.isValid()) {
            throw new BadRequestException("Validation failed for Quiz: " + String.join(", ", res.errorMessages()));
        }
    }
}
